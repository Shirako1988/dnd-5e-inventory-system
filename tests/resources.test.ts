...
test('long rest includes short rest; dawn remains separate; never exceeds maximum',()=>{const short={...resource,current:2,reset:'shortRest' as const};assert.equal(recoverResource(short,'longRest').current,10);assert.equal(recoverResource(short,'dawn').current,2);assert.equal(recoverResource({...short,reset:'dawn'},'longRest').current,2);assert.equal(recoverResource({...short,reset:'dawn',recovery:'1d6+1'},'dawn',()=>0.99).current,9);assert.equal(recoverResource({...short,current:9,recovery:'1d6+1'},'shortRest',()=>0.99).current,10);assert.equal(recoverResource({...short,reset:'none'},'longRest').current,2);});
test('retry rolls repeat, separate specimens get their own rolls',()=>{const a=seededRandom(123),b=seededRandom(123);const roll=()=>Array.from({length:20},()=>recoverResource({...resource,current:0,reset:'dawn',recovery:'1d6'},'dawn',a).current);const first=roll();assert.deepEqual(first,Array.from({length:20},()=>recoverResource({...resource,current:0,reset:'dawn',recovery:'1d6'},'dawn',b).current));assert.ok(new Set(first).size>1);});
