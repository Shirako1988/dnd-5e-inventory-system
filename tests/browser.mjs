...
try{
 ...
 const staffId=(await records()).find(i=>i.name==='Test-Stab').id;
 await page.locator(`[data-item-id="${staffId}"]`).getByRole('button',{name:'Bearbeiten',exact:true}).click();
 let resourceEditor=page.locator(`[data-item-id="${staffId}"]`);
 await resourceEditor.getByRole('button',{name:'Ressource entfernen',exact:true}).first().click();
 await resourceEditor.getByRole('button',{name:'Speichern',exact:true}).click();
 await page.waitForFunction(id=>JSON.parse(localStorage.getItem('dnd-inventory-items')).find(i=>i.id===id)?.resources.length===1,staffId);
 assert.equal((await records()).find(i=>i.id===staffId).resources[0].id,'dawn');
 await resourceEditor.getByRole('button',{name:'Bearbeiten',exact:true}).click();
 await resourceEditor.getByRole('button',{name:'Ressource entfernen',exact:true}).click();
 await resourceEditor.getByRole('button',{name:'Speichern',exact:true}).click();
 await page.waitForFunction(id=>JSON.parse(localStorage.getItem('dnd-inventory-items')).find(i=>i.id===id)?.resources.length===0,staffId);
 assert.equal((await records()).find(i=>i.id===staffId).quantity,2);
 assert.equal((await records()).find(i=>i.id===staffId).resourceVersion,1);
 await page.reload();await page.getByRole('heading',{name:'Items in dieser Tasche'}).waitFor();
 assert.equal((await records()).find(i=>i.id===staffId).resources.length,0);
 assert.equal(await page.getByRole('alert').count(),0);
 console.log('PASS: editor removes one resource and the last resource; quantity and removal survive reload');
